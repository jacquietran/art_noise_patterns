...
iteration_id <- "noise_patterns_0047"
palette <- c("#FF8360", "#E8E288", "#7DCE82", "#3CDBD3", "#00FFF5")
initial_seed <- 587047
...
grid <- create_noise_grid(
  ...
  fracture_noise = "spheres", length_out = length_out_value)
...
grid_chess <- create_overlay(
  ...
  shear_value = shear_vec[1], length_out = length_out_value)

grid_chess2 <- create_overlay(
  rotate_value1 = rotate_vec[3], rotate_value2 = rotate_vec[4],
  shear_value = shear_vec[2], length_out = length_out_value)
