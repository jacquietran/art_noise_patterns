...
iteration_id <- "noise_patterns_0013"
...
grid <- long_grid(x = seq(0, 10, length.out = 1000), 
  ...
  mutate(
    ...
    worley_frac = fracture(gen_cubic, ridged, octaves = octaves_vec[1], x = x, y = y, 
...
grid2 <- long_grid(x = seq(0, 10, length.out = 1000), 
  ...
  mutate(
    ...
    worley_frac = fracture(gen_cubic, ridged, octaves = octaves_vec[2], x = x, y = y, 
