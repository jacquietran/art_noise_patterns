...
iteration_id <- "noise_patterns_0007"
gradient_colours <- c("#987284", "#75B9BE", "#D0D6B5", "#F9B5AC", "#EE7674")
initial_seed <- 587007
...
set.seed(initial_seed)
rotate_vec <- sample(4:30, 4, replace = FALSE)

set.seed(initial_seed)
shear_vec <- sample(2:20, 2, replace = FALSE)
...
grid <- long_grid(x = seq(0, 10, length.out = 1000), 
  ...
  mutate(
    ...
    worley_frac = fracture(gen_worley, ridged, octaves = octaves_vec[1], x = x, y = y, 
...
grid2 <- long_grid(x = seq(0, 10, length.out = 1000), 
                   y = seq(0, 10, length.out = 1000)) %>% 
  mutate(
    x1 = x + gen_cubic(x, y) / 2, 
    y1 = y + gen_cubic(x, y) / 2,
    worley = gen_worley(x, y, value = 'distance', seed = seed_vec[2]),
    worley_frac = fracture(gen_value, ridged, octaves = octaves_vec[2], x = x, y = y, 
...
grid_chess <- long_grid(
  ...
  mutate(
    ...
    chess = gen_perlin(trans$x, trans$y))
